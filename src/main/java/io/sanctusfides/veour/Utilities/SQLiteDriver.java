...
public class SQLiteDriver {
    ...
    public String[] getLatAndLong(String city, String state) {
        System.out.println("Starting values : " + city + " " + state);
        String[] arr = new String[2];
//        String query = "SELECT lat, long FROM locations WHERE city = ? AND state = ?";
        String query = "SELECT * FROM locations WHERE city = ? AND state = ?";
...
//                System.out.println("STATEMENT: " + preparedStatement.getParameterMetaData());
                try (ResultSet results = preparedStatement.executeQuery()) {
                    while (results.next()) {
                        System.out.println("SUCCESS");
                        System.out.println(results.getString("lat"));
                        System.out.println(results.getString("long"));
